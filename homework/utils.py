...
# from controller import steering_pid, speed_pid



# from controller import load_q_table
...
class PyTux:
    ...
    def rollout(self, track, controller, planner=None, max_frames=2000, verbose=False, data_callback=None):
        """
        ...
        for t in range(max_frames):
            ...
            if np.isclose(kart.overall_distance / track.length, 1.0, atol=2e-3):
                if verbose:
                    ...
                    run_results.append((t, kart.overall_distance / track.length))
                break
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n', '--n_images', default=100000, type=int) #10000
    parser.add_argument('-m', '--steps_per_track', default=200000, type=int) # 20000
    ...
    for track in args.track:
        ...
        while n < args.steps_per_track and current_runs < max_runs:
            steps, how_far = pytux.rollout(track, noisy_control, max_frames=3000, verbose=args.verbose, data_callback=collect)
            print(steps, how_far)
...
print("Run results:")
for time, percent in run_results:
    print(f"Time: {time}, Percent: {percent}")
